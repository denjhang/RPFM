...
static void ResetState(void) {
    ...
    s_soloCh = -1;
    if (s_muteMode == 1 && s_connected) rpfm_set_mute(0, -1);
...
static DWORD WINAPI VGMStreamThread(LPVOID param) {
    ...
    // Initial patch: apply mute state to all data before prefill (host mode only)
    if (s_muteMode == 0)
...
static void ApplyChannelMute(int i) {
    ...
    // Host mode: write registers directly
    if (ch < 3) {
...
static void RenderSidebar(void) {
    ...
    // Mute Mode
    ImGui::Spacing();
    ImGui::TextDisabled("Mute Mode");
    if (ImGui::IsItemHovered()) ImGui::SetTooltip(
        "Host: patch VGM data on PC (delay on unmute)\n"
        "Firmware: intercept registers in device (instant)");
    if (ImGui::RadioButton("Host##mutemode", &s_muteMode, 0)) SaveConfig();
    ImGui::SameLine();
