...
- **电平表**：统一音量条（tone/noise/env/DAC 全模式响应）+ dB 刻度，mute/solo
- **寄存器表**：实时显示影子寄存器，tone/noise/mixer/volume/envelope 全可视化
- **示波器**：可配置波形显示
- **播放控制**：VGM 文件加载、播放/暂停/停止、进度跳转（seek）、循环展开
- **暂停/跳转**：暂停 = 停止播放 + 记住位置，恢复 = seek 到暂停位置；进度条拖动 = 停止→静默快进→从目标位置重启
- **循环控制**：VGM 循环段在内存中展开为线性数据，循环计数可调，支持循环开关
- **侧边栏配置**：PIO 延时调整、缓冲区大小 (512B-8KB)、播放模式切换、时钟校正等
- **Tick 显示**：固件播放位置 / 总 tick，时间同步
- **播放稳定性**：HID 写入重试、多媒体定时器轮询、失败自动恢复
...
- **2026.05.14** — 缓冲模式暂停/跳转/循环：暂停复用 stop+seek、循环展开为线性数据、进度条拖动 seek
- **2026.05.13** — 播放稳定性优化：HID 重试、多媒体定时器、缓冲区 512B-8KB、即时可视化
- **2026.05.13** — AY8910 电平表改造：统一音量条、DAC/ENV 模式显示
- **2026.05.13** — GPIO 扩展规划 (RP2350A/B、触摸屏、SD 卡、USB Host MIDI)
